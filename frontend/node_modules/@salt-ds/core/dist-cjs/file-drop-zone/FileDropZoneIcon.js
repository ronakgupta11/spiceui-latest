'use strict';

var jsxRuntime = require('react/jsx-runtime');
var React = require('react');
var SemanticIconProvider = require('../semantic-icon-provider/SemanticIconProvider.js');
var StatusIndicator = require('../status-indicator/StatusIndicator.js');

const FileDropZoneIcon = React.forwardRef(function FileDropZoneIcon2({ status, size = 2, ...rest }, ref) {
  const iconProps = {
    ref,
    size,
    ...rest
  };
  const { UploadIcon } = SemanticIconProvider.useIcon();
  return status ? /* @__PURE__ */ jsxRuntime.jsx(StatusIndicator.StatusIndicator, { status, ...iconProps }) : /* @__PURE__ */ jsxRuntime.jsx(UploadIcon, { ...iconProps });
});

exports.FileDropZoneIcon = FileDropZoneIcon;
//# sourceMappingURL=FileDropZoneIcon.js.map
